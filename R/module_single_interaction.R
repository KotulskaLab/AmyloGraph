...
server_single_interaction <- function(id, interactions) {
  moduleServer(id, function(input, output, session) {
    observe({
      ...
      output[["interactor_sequence"]] <- renderText(prettify_sequence_output(as.character(selected_interaction[["interactor_sequence"]])))
      output[["interactee_name"]] <- renderText(selected_interaction[["interactee_name"]])
      output[["interactee_sequence"]] <- renderText(prettify_sequence_output(as.character(selected_interaction[["interactee_sequence"]])))
