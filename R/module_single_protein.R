...
server_single_protein <- function(id, edge_data, node_data, protein_data) {
  moduleServer(id, function(input, output, session) {
    ...
    subtables[["interactees"]] <- reactive_subtable_data(edge_data, ns, input, from_id, interactee_name)
    subtables[["interactors"]] <- reactive_subtable_data(edge_data, ns, input, to_id, interactor_name)
    
    output[[NS("interactees", "table")]] <- render_interactions_subtable(subtables[["interactees"]])
    output[[NS("interactors", "table")]] <- render_interactions_subtable(subtables[["interactors"]])
    
    interactees_proxy <- dataTableProxy(NS("interactees", "table"))
    interactors_proxy <- dataTableProxy(NS("interactors", "table"))
